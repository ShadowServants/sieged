...
func BuildTestFlagHandler() *FlagHandler {
	...
	fHandler.TeamNum = 3
	fHandler.Points.SetPoints("1", &Points{0, 0, 1700})
	fHandler.Points.SetPoints("2", &Points{0, 0, 1700})
	fHandler.Points.SetPoints("3", &Points{0, 0, 1700})
...
func TestFlagHandler_calcDelta_smaller(t *testing.T) {
	...
	Convey("Check delta if attacker pts < victim_pts", t, func() {
		So(delta, ShouldAlmostEqual, flagHandler.TeamNum)
	})
}
...
func TestFlagHandler_calcDelta_almost_equal(t *testing.T) {
	...
	Convey("Check delta if attacker pts < victim_pts", t, func() {
		So(delta, ShouldAlmostEqual, 14.759984857605815)
	})
}
...
func TestFlagHandler_calcDelta_2(t *testing.T) {
	...
	Convey("Check delta if attacker pts > victim pts", t, func() {
		So(delta, ShouldAlmostEqual, 9.21645877703273)
	})
...
func TestFlagHandler_calc(t *testing.T) {
	Convey("Check team 1 attacks team 2", t, func() {
		...
		So(attackerData.points.Plus, ShouldEqual, 1)
		So(attackerData.points.Minus, ShouldEqual, 0)
		So(victimData.points.Plus, ShouldEqual, 0)
		So(victimData.points.Minus, ShouldEqual, 1)
		So(attackerData.points.Points, ShouldAlmostEqual, 1703)
		So(victimData.points.Points, ShouldAlmostEqual, 1697)
	})
	Convey("Check team 2 attacks team 1", t, func() {
		...
		res := flagHandler.calc(attacker, victim)
		So(res, ShouldAlmostEqual, 3)
		So(flagHandler.Teams[attacker].points.Plus, ShouldEqual, 1)
		So(flagHandler.Teams[attacker].points.Minus, ShouldEqual, 1)
		So(flagHandler.Teams[victim].points.Plus, ShouldEqual, 1)
		So(flagHandler.Teams[victim].points.Minus, ShouldEqual, 1)
		So(flagHandler.Teams[attacker].points.Points, ShouldAlmostEqual, 1700)
		So(flagHandler.Teams[victim].points.Points, ShouldAlmostEqual, 1700)
	})
	Convey("Check points zero", t, func() {
		attacker := 1
		victim := 2
		flagHandler.Teams[attacker].points.Points = 1
		flagHandler.Teams[victim].points.Points = 1
		flagHandler.calc(attacker, victim)
		So(flagHandler.Teams[victim].points.Points, ShouldEqual, 0)
	})
...
func TestFlagHandler_CheckFlag(t *testing.T) {
	...
	Convey("Create flag and check it", t, func() {
		flag := flagdata.FlagData{Team: 2, Round: 2}
...
func TestFlagHandler_ValidateFlag(t *testing.T) {
	...
	flag := flagdata.FlagData{Team: 1, Round: 1}
	flagHandler.Flags.SetFlagData("flagflag", &flag)
...
func TestFlagHandler_HandleRequest(t *testing.T) {
	...
	fd1 := flagdata.FlagData{Team: 1, Round: 1}
	fd2 := flagdata.FlagData{Team: 1, Round: 2}
