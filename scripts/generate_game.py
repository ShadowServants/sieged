...
@click.option('--platform', default='darwin64', help='Platform to run available: darwin64/linux64')
def create_service(path, platform):
    ...
    with click.progressbar(binaries) as progress:
        for binary in progress:
            from_path = "bin/{}/{}".format(platform, binary)
    ...
    FLAG_HANDLER_PORT = int(click.prompt("Enter port for flag handler", default=str(FLAG_HANDLER_PORT)))
...
@click.option('--platform', default='darwin64', help='Platform to run available: darwin64/linux64')
@click.option('--flags', default='http', help='Interface to flags. available: http/tcp/both ')
@click.pass_context
def generate_game(ctx, path, num, platform, flags):
    ...
    if flags == 'both':
        programs.append('tcp')
        programs.append('http')
    else:
        programs.append(flags)
    programs = ['flag_router_' + x for x in programs]

    supervisor_conf = render_template('supervisord.conf', routers=programs)
    save_config_file(path + "/supervisord.conf", supervisor_conf)
    ...
    with click.progressbar(programs) as progress:
        for binary in progress:
            from_path = "bin/{}/{}".format(platform, binary)
